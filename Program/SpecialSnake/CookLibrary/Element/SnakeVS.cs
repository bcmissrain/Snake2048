...
namespace CookLibrary.VSGame
{
    ...
    class Body:BaseBody
    {
        ...
        public Body(string sourceName)
            : base(sourceName, new Rectangle(0, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH),
                new Rectangle(World1.SNAKE_WIDTH, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH))
        {
    ...
    class Head:BaseBody
    {
        public Head(string sourceName)
            : base(sourceName, new Rectangle(0, 0, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH)
                , new Rectangle(World1.SNAKE_WIDTH, 0, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH))
        {
            this.position = position;
        }
    ...
    class Tail :BaseBody
    {
        public Tail(string sourceName)
            : base(sourceName, new Rectangle(0, World1.SNAKE_WIDTH * 2, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH)
                , new Rectangle(World1.SNAKE_WIDTH, World1.SNAKE_WIDTH * 2, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH))
        {
            this.position = position;
        }
    }
    ...
    class Food : BaseBody 
    {
        public Food()
        {
            this.bodyTexture = SAGlobal.Content.Load<Texture2D>("Images/Snake0");
            this.direction = BodyDirection.Up;
            this.sourceRectU = this.sourceRectL = new Rectangle(World1.SNAKE_WIDTH * 2, 0, World1.SNAKE_WIDTH, World1.SNAKE_WIDTH);
        }
    ...
    class Snake
    {
        ...
        public int ScoreDelta { get { return World1.ScoreDelta; } }
        ...
        public void Update(World1 world)
        {
            if (counter >= MAX_COUNT)
            {
                AutoTurnDirection();
                counter = 0;
                CheckEat(World1.foods);
                if (ifEat)
                {
                    Eat();
                }
        ...
        {
            //在世界数组中删除尾巴
            World1.worldMap[(int)(tail.position.X), (int)(tail.position.Y)] = ElementType.Ground; //世界数组更改头

            UpdateTail();
            UpdateBodys();

            //在世界数组中将未移动的头的块位置设置为身体
            World1.worldMap[(int)(head.position.X), (int)(head.position.Y)] = ElementType.SnakeBody; //世界数组更改头

            UpdateHead();
            CheckMove();
            //在世界数组中将已经移动的头的位置设置为头
            World1.worldMap[(int)(head.position.X), (int)(head.position.Y)] = ElementType.SnakeHead; //世界数组更改头
        }
        /// <summary>
        /// 最先更新尾部数据
        /// </summary>
        ...
        {
            for (int i = 0; i < World1.foods.Count; i++)
            {
                //5.删除所有食物项（之后删），并且修改世界数组
                World1.worldMap[(int)(World1.foods[i].position.X), (int)(World1.foods[i].position.Y)] = ElementType.Ground;
            }
            World1.foods.Clear();//删除了所有内容

            BaseBody temp = new Body(this.sourceName);
            temp.Init(head);
            if (IfHaveBody)
            {
            ...
            CheckMove();
            World1.worldMap[(int)(head.position.X), (int)(head.position.Y)] = ElementType.SnakeHead; //世界数组更改头
            World1.worldMap[(int)(bodys[0].position.X), (int)(bodys[0].position.Y)] = ElementType.SnakeBody;//世界数组更改新身体，旧身体与尾不修改
        ...
        protected void CheckMove()
        {
            ...
            {
                head.position.X  = World1.WORLD_WIDTH-1;
            }
            else if (head.position.X > World1.WORLD_WIDTH-1)
            {
                head.position.X = 0;
            }
            if (head.position.Y < 0)
            {
                head.position.Y = World1.WORLD_HEIGHT - 1;
            }
            else if (head.position.Y > World1.WORLD_HEIGHT - 1)
            {
                head.position.Y = 0;
            }
        }
