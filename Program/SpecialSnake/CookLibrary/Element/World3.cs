...
namespace CookLibrary.LRGame
{
    ...
    class World1
    {
        ...
        public bool IfLose()
        {
            ...
            int temp = Math.Abs(snake0.bodys.Count() - snake1.bodys.Count());
            if (temp > 1)
            {
                return true;
            }
        ...
        private bool IfCollide()
        {
            ...
            {
                if (tempPos0 == snake0.bodys[i].position || tempPos1 == snake0.bodys[i].position)
                {
                    return true;
                }
            }
            ...
            {
                if (tempPos0 == snake1.bodys[i].position || tempPos1 == snake1.bodys[i].position)
                {
                    return true;
                }
            }
            if (tempPos0 == snake0.tail.position||tempPos0 == snake1.tail.position
                ||tempPos1 == snake0.tail.position||tempPos1 == snake1.tail.position)
            {
                return true;
            }
            return false;
